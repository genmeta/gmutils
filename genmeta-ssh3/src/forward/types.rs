...
impl FromStr for DynamicForwardEndpoint {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let port = port
            .parse::<u16>()
            .map_err(|_| format!("Invalid port `{port}` in dynamic forward bind address `{s}`"))?;
        ...
        let addresses = match host {
            ...
            ipaddr => {
                let ipaddr = ipaddr.parse::<IpAddr>().map_err(|_| {
                    format!("Invalid host `{host}` in dynamic forward bind address `{s}`")
                })?;
...
impl FromStr for LocalForwardRule {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let rule = s
            .parse::<super::parser::LocalForwardRule>()
            .map_err(|e| format!("Failed to parse local forward rule `{s}`: {e:?}"))?;

        let remote_address = match rule.remote {
            super::parser::RemoteEndpoint::Host { host, port } => BindAddress::Host { host, port },
            super::parser::RemoteEndpoint::Unix { path } => BindAddress::Unix { path },
        };

        let local_addresses = match rule.local {
            super::parser::LocalEndpoint::Addr(socket_addr) => vec![BindAddress::Host {
                host: socket_addr.ip().to_string(),
                port: socket_addr.port(),
            }],
            super::parser::LocalEndpoint::Port(port) => vec![
                BindAddress::Host {
            ...
            super::parser::LocalEndpoint::Unix(path) => vec![BindAddress::Unix { path }],
        };
...
impl FromStr for RemoteForwardRule {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let rule = s
            .parse::<super::parser::RemoteForwardRule>()
            .map_err(|e| format!("Failed to parse remote forward rule `{s}`: {e:?}"))?;
