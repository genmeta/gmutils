    ...
    for relative in DEB_SEARCH_DIRS {
            ...
            .whatever_context(format!("failed to read entry in {}", directory.display()))?
        {
            ...
            let filename = path
                ...
                .to_string();
            let (package, version) = parse_deb_filename(&filename)?;
            debs.push(DebSource {
                package,
                version,
                filename,
...
mod tests {
    use super::{
        ...
        parse_deb_filename, pool_path, sign_suite_release_script, validate_path_segment,
    };

    #[test]
    fn pool_path_uses_debian_pool_layout() {
        assert_eq!(
            pool_path("gmutils", "gmutils_0.5.1-1_amd64.deb"),
            std::path::PathBuf::from("pool/main/g/gmutils/gmutils_0.5.1-1_amd64.deb")
        );
    }
    ...
    fn deb_filename_parser_extracts_package_and_upstream_version() {
        let (package, version) =
            parse_deb_filename("gmutils_0.5.1-1_amd64.deb").expect("filename should parse");
