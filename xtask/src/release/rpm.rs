...
fn release_version(rpms: &[RpmSource]) -> Result<String, Whatever> {
    let version = rpms
        ...
        .clone();
    snafu::ensure_whatever!(
        rpms.iter().all(|rpm| rpm.package == PACKAGE_NAME),
...
mod tests {
    ...
    fn rpm_artifact_path_uses_flat_package_version_layout() {
        assert_eq!(
            rpm_artifact_path("0.5.1", "gmutils-0.5.1-1.x86_64.rpm"),
            PathBuf::from("gmutils/0.5.1/gmutils-0.5.1-1.x86_64.rpm")
        );
    }
    ...
    fn rpm_filename_parser_extracts_package_and_upstream_version() {
        let (package, version) =
            parse_rpm_filename("gmutils-0.5.1-1.x86_64.rpm").expect("filename should parse");

        assert_eq!(package, "gmutils");
    ...
    fn release_version_rejects_mixed_rpm_versions() {
        let rpms = vec![
            RpmSource {
                ...
                source: PathBuf::from("gmutils-0.5.1-1.x86_64.rpm"),
            },
            RpmSource {
                package: "gmutils".to_string(),
                version: "0.5.2".to_string(),
                filename: "gmutils-0.5.2-1.aarch64.rpm".to_string(),
                source: PathBuf::from("gmutils-0.5.2-1.aarch64.rpm"),
            },
        ];
    ...
    fn manifest_merge_preserves_non_rpm_entries_and_replaces_stale_rpm() {
        let existing = ReleaseManifest {
            ...
            artifacts: vec![
                ArtifactEntry {
                    ...
                    path: "pool/main/g/gmutils/gmutils_0.5.1-1_amd64.deb".to_string(),
        ...
        let merged = merge_rpm_manifest(
            ...
            "0.5.1",
            vec![RpmInfo {
                path: "gmutils/0.5.1/gmutils-0.5.1-1.x86_64.rpm".to_string(),
        ...
        assert!(
            merged
                ...
                .any(|artifact| artifact.root == ArtifactRoot::Apt
                    && artifact.path == "pool/main/g/gmutils/gmutils_0.5.1-1_amd64.deb")
        );
        assert!(
            !merged
                .artifacts
        ...
        assert!(
            merged
                ...
                .any(|artifact| artifact.root == ArtifactRoot::Rpm
                    && artifact.path == "gmutils/0.5.1/gmutils-0.5.1-1.x86_64.rpm"
                    && artifact.immutable)
        );
    }
}
