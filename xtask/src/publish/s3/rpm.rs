...
impl RpmStageContainer {
    async fn start(repository: &Path) -> Result<Self, Whatever> {
        ...
        let container_name = "gmutils-xtask-rpm-publish";
...
async fn ensure_rpm_stage_image(docker: &Docker) -> Result<(), Whatever> {
    ...
    let container_name = "gmutils-xtask-rpm-publish-setup";
    remove_container_if_exists(docker, container_name).await;
    let container = docker
        .create_container(
            Some(
...
mod tests {
    ...
    fn manifest_arch_replaces_remote_same_arch_and_preserves_others() {
        let remote = vec![
            remote_payload("gmutils", "0.5.1", "x86_64"),
            remote_payload("gmutils", "0.5.1", "aarch64"),
        ];
        let local = vec![local_payload("gmutils", "0.5.2", "x86_64")];
    ...
    fn rpm_payload_key_uses_package_version_layout() {
        let key = super::rpm_payload_key(
            "rpm/gmutils",
            "gmutils",
            "0.5.2",
            "gmutils-0.5.2-1.x86_64.rpm",
        );
        assert_eq!(key, "rpm/gmutils/gmutils/0.5.2/gmutils-0.5.2-1.x86_64.rpm");
    }
