...
impl AptStageContainer {
    async fn start(repository: &Path, options: &AptPublishOptions) -> Result<Self, Whatever> {
        ...
        let container_name = "gmutils-xtask-apt-publish";
...
async fn ensure_apt_stage_image(docker: &Docker) -> Result<(), Whatever> {
    ...
    let container_name = "gmutils-xtask-apt-publish-setup";
    remove_container_if_exists(docker, container_name).await;
    let container = docker
        .create_container(
            Some(
...
mod tests {
    ...
    fn publishable_deb_payloads_skip_equal_and_older_versions() {
        let remote = vec![remote_package_entry("gmutils", "0.5.2-1", "amd64")];
        let local = vec![
            local_payload("gmutils", "0.5.2-1", "amd64"),
            local_payload("gmutils", "0.5.1-1", "amd64"),
        ];
    ...
    fn publishable_deb_payloads_compare_against_latest_remote_version() {
        let remote = vec![
            remote_package_entry("gmutils", "0.5.1-1", "amd64"),
            remote_package_entry("gmutils", "0.5.2-1", "amd64"),
        ];
        let local = vec![local_payload("gmutils", "0.5.2-2", "amd64")];
    ...
    fn deb_payload_key_uses_prefix_and_pool_path() {
        let key = deb_payload_key("apt/gmutils", "gmutils", "gmutils_0.5.2-1_amd64.deb");
        assert_eq!(
            key,
