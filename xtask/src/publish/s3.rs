...
pub async fn run(options: S3Options, targets: Vec<OsString>) -> Result<(), Whatever> {
    ...
    for target in targets {
        match target {
            ...
            S3Target::Scoop(target) => scoop::run(&resolved_options, &client, target).await?,
...
fn target_format_to_target(
    ...
    match target {
        ...
        S3TargetFormat::Scoop => {
            let scoop = contract.destination.scoop.as_ref().ok_or_else(|| {
                target_error(
                    ErrorKind::MissingRequiredArgument,
                    "destination.scoop is required",
                )
            })?;
            Ok(S3Target::Scoop(ScoopPublishTarget {
                prefix: parse_prefix(section_name, &scoop.prefix)?,
                public_base_url: parse_public_base_url(section_name, &scoop.public_base_url)?,
            }))
        }
        S3TargetFormat::Deb => {
...
mod tests {
    ...
    fn parses_destination_contract_targets_without_per_target_options() {
        ...
        let targets = ["rpm", "brew", "scoop"].map(std::ffi::OsString::from);
        ...
        assert!(matches!(parsed[2], S3Target::Scoop(_)));
    }

    #[tokio::test]
    async fn s3_client_calculates_request_checksums_only_when_required() {
        ...
        let client = client(&options).await.expect("client should build");
