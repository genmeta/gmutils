...
impl Binds {
        ...
        I: IntoIterator<Item = &'a str> + Clone,
    {
        ...
        let mut push_bind_uri = |bind_uri: BindUri| {
            ...
            // Normalize a bare `/` to `None` to represent "no path-and-query"
            // in stored state. Many callers treat `None` as the absence of an
            // explicit path-and-query while displaying it as `/`, so keep
            // storage normalized to `None` for consistent conflict semantics.
            let path_and_query =
