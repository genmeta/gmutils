...
- `genmeta-identity` now embeds the certificate server base URL from the
  build-time `DHTTP_CERT_SERVER_URL` environment variable. Runtime
  `CERT_SERVER_URL` overrides are no longer used.
